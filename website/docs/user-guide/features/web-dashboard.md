...
Authenticated API calls accept the ephemeral dashboard token through `X-Hermes-Session-Token: TOKEN` (preferred) or the legacy `Authorization: Bearer TOKEN` header.
